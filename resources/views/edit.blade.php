...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            ...
            <div>
              <form id="myForm" enctype="multipart/form-data" method="POST" action="{{ route('folder.update', $folder->id) }}">
                @csrf
                @method('PUT')
                <div class="form-group">
                  <h5>Folder's Name</h5>
                  <input type="text" class="form-control" id="name" name="name" aria-describedby="folderHelp" placeholder="Enter folder's name" value="{{ $folder->name }}">
                ...
                <div class="row m-0 pb-4" id="gallery_preview">
                  @foreach($folder->images as $image)
                    <div class="col-md-3 mt-4 pip" style="text-align: center;">
                      <img class="img-fluid" style="object-fit: cover; width: 300px; max-height: 400px;" src="{{ env('AWS_S3_URL').$image->path }}">
                      <div id="image_{{ $loop->iteration }}" onclick="deletePreview(this.id)">
                        <i style="color:#145CA8;font-size:20px" class="fas fa-minus-circle mt-2 remove"></i>
                      </div>
                      <input name="images[]" value="{{ $image->filename }}" hidden>
                    </div>
                  @endforeach
                </div>
